...
class DIP:
    ...
    def out_loss(self,LOSS, out, img_noisy_torch):  
        ...
        if LOSS == 'mse':  
            ...
            global pn, alpha
            pn = (1-pn_np/25)
            alpha_tensor = torch.log(mse(out.detach(), img_noisy_torch).type(torch.FloatTensor)/(0.1*math.log(math.sqrt(max(i,100.0)),10)*tv(out.detach())/NumPix).type(torch.FloatTensor))
    ...
    def closure(self):
        ...
        print ('Iteration %05d    pn %f   alpha: %f    Loss %f   PSNR_noisy: %f   PSRN_gt: %f PSNR_gt_sm: %f' % (i, pn, alpha, total_loss.item(), psrn_noisy, psrn_gt, psrn_gt_sm), '\r', end='')
